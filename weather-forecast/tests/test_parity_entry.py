...
(storage.load_open_positions, market_client.get_available_depth_usd,
market_client.estimate_slippage) and must produce byte-for-byte identical
EntryDecision objects across every gate 1-9 plus the exploratory-sizing
approve path. A field-level mismatch here means the replica has drifted
from the live gate logic.
...
# (label, ev, open_count_for_bucket, depth_usd, slippage, min_net_ev)
# open_count_for_bucket=None replicates "open positions unreadable" (gate 2).
CASES = [
    ("gate1_raw_edge_veto",        make_ev(0.95, 0.30), 0,    1000.0, 0.01, 0.15),
    ("gate2_unreadable_count",     make_ev(0.55, 0.35), None, 1000.0, 0.01, 0.15),
    ("gate3_per_bucket_cap",       make_ev(0.55, 0.35), 1,    1000.0, 0.01, 0.15),
    ("gate4_kelly_le_zero",        make_ev(0.20, 0.35), 0,    1000.0, 0.01, 0.15),
    ("gate5_depth_unknown",        make_ev(0.55, 0.35), 0,    None,   0.01, 0.15),
    ("gate6_depth_too_thin",       make_ev(0.55, 0.35), 0,    2.0,    0.01, 0.15),
    ("gate7_slippage_gate",        make_ev(0.55, 0.35), 0,    1000.0, 0.40, 0.15),
    ("gate8_net_ev_at_size",       make_ev(0.55, 0.35), 0,    1000.0, 0.01, 0.90),
    ("gate9_approved",             make_ev(0.55, 0.35), 0,    1000.0, 0.01, 0.15),
    ("gate9_approved_exploratory", make_ev(0.55, 0.35, station="WMKK"), 0, 1000.0, 0.01, 0.15),
]


@pytest.mark.parametrize(
    "ev,open_count,depth,slip,min_net_ev",
    [case[1:] for case in CASES],
    ids=[case[0] for case in CASES],
)
def test_entry_decision_field_parity(monkeypatch, ev, open_count, depth, slip, min_net_ev):
    ...
    sim = entry_sim.evaluate_entry_sim(
        ev=ev, token_id="TOKEN-1", open_count_for_bucket=open_count, depth_usd=depth,
